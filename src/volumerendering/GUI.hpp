...
static void update_camera(Camera_VR& cam, double phi, double theta, float base_dist, int zoom)
{
    float3 cam_dir;
...
#include "../../libs/vendor/imgui/imgui.h"
#include "../../libs/vendor/imgui/backends/imgui_impl_dx9.h"
#include "../../libs/vendor/imgui/backends/imgui_impl_win32.h"
#include <d3d9.h>
#include <tchar.h>
...
private:
    ...
    void DrawMainMenu() {
        ...
        ImGui::Text("Camera_VR Pos: (%.5f, %.5f, %.5f) ", pos.x, pos.y, pos.z);
        float3 l;
        ...
        ImGui::SliderFloat("Camera_VR Exposure", &exposure, 0, 2);
...
public:
    GUIs(const Camera_VR& cam): camera(cam) {  }
...
void RunGUI(Camera_VR& cam, VolumeRender& volume, float3 lightDir = normalize({ 1,1,1 }), float3 lightColor = { 1,1,1 }, float3 scatter_rate = { 1, 1, 1 }, float alpha = 1, float multiScatterNum = 10, float g = 0, bool hideConsole = false) {
#ifdef GUI
    if (hideConsole) {
        HWND hwnd;
