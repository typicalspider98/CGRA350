...
float3 Camera_VR::None(float3 color) {
    return color;
}

float3 Camera_VR::Gamma(float3 color) {
    return pow(color, 1.0f / 2.2f);
}

float3 Camera_VR::ACES(float3 color) {
...
Camera_VR::Camera_VR(VolumeRender& volume, string name, float3 position, int resolution) {
    ori = position;
...
void Camera_VR::SetPosition(float3 position) {
    ori = position;
    forward = normalize(-ori);
    right = normalize(cross({ 0,1,0 }, forward));
    up = cross(forward, right);
...
void Camera_VR::SetVolume(VolumeRender& volume) {
    this->volume = &volume;
}

void Camera_VR::Render(float3 lightDir, float3 lightColor, float alpha, int multiScatter, float g, int sampleNum, ToneMapFunction tone, VolumeRender::RenderType rt, float exp) {
...
void Camera_VR::RenderToFile(string path, float3 lightDir, float3 lightColor, float alpha, int multiScatter, float g, int sampleNum, ToneMapFunction tone, VolumeRender::RenderType rt, float exp) {
