...
__device__ float3 GetSample(float3 ori, float3 dir, float3 lightDir, float3 lightColor, float scatter, float alpha, int multiScatter, float g, int sampleNum) {
...
//template<int type, int BatchSize, int StepNum>
//__device__ float4 NNPredict(float3 ori, float3 dir, float3 lightDir, float3 lightColor, float alpha, float g) {
//
//    dir = normalize(dir);
//    lightDir = normalize(lightDir);
//
